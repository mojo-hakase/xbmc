...
{
  bool parsed = false;
  std::map<uint32_t,uint64_t> seekPosition;
};
struct MatroskaSegment
{
...
bool ParseMatroskaSegmentInfo(MatroskaFile *mkv, uint64_t tagEnd, CDVDInputStream *input)
{
...
bool ParseMatroskaSeekHead(MatroskaFile *mkv, uint64_t tagEnd, CDVDInputStream *input)
{
  uint32_t seekID = 0;
  uint64_t seekPos = 0;
  uint8_t byte;
...
bool ParseMatroskaChapterAtom(MatroskaEditionEntry *edition, uint64_t tagEnd, CDVDInputStream *input)
{
...
bool ParseMatroskaEditionEntry(MatroskaFile *mkv, uint64_t tagEnd, CDVDInputStream *input)
{
...
bool ParseMatroskaChapters(MatroskaFile *mkv, uint64_t tagEnd, CDVDInputStream *input)
{
  while (!input->IsEOF() && input->Seek(0, SEEK_CUR) < tagEnd)
  {
    uint32_t id = ReadEbmlTagID(input);
